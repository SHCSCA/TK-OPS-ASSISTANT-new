...
- 桌面启动链路已支持宿主先探测 runtime 实际地址，开发脚本在默认端口被占用时自动切换可用端口
...
- 旧壳保留页面基线为 `35` 个，新壳当前左侧菜单按产品范围收口为 `34` 个页面
- 已迁移页面为 `14` 个，其余页面先进入占位路由，避免再发生漏页
- 迁移矩阵已更新：
...
- 不再跳着挑页面深挖
- 从当前仍保留的第一页开始，按菜单顺序做旧壳 1:1 深度迁移与代码转换（`device-management`、`scheduled-publish`、`data-collector`、`creative-workshop`、`ai-content-factory`、`video-editor` 已完成）
- 每页固定顺序：旧壳基线冻结 -> spec -> 页面结构 -> runtime API -> 交互 -> 样式 -> 测试
- 后续页面统一要求：结构、文案、动作、数据派生、详情关系、样式约束都必须对齐旧壳，不接受只迁“大意”的改写
...
- 不再接受轻量换皮式迁移，后续保留页面统一按旧壳做 1:1 深度迁移与代码转换
