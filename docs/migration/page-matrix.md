...
- 当前执行顺序：
  1. 主体框架与全局配置先迁移
  2. 左侧菜单与路由骨架一次补齐
  3. 按菜单顺序逐页做旧壳 1:1 深度迁移与代码转换，页面、数据、交互、样式、测试必须整页闭环
...
| device-management | 设备管理 | 运营管理 | `/device-management` | P0 | 已迁移 |
| asset-center | 素材中心 | 运营管理 | `/asset-center` | P1 | 已迁移 |
| task-queue | 任务队列 | 自动化任务 | `/tasks` | P0 | 已迁移 |
| scheduled-publish | 定时发布 | 自动化任务 | `/scheduled-publish` | P1 | 已迁移 |
| data-collector | 数据采集助手 | 自动化任务 | `/data-collector` | P1 | 已迁移 |
| creative-workshop | 创意工坊 | 内容与 AI | `/creative-workshop` | P1 | 已迁移 |
| visual-lab | 可视化实验室 | 内容与 AI | `/visual-lab` | P2 | 已注册菜单与占位路由 |
| ai-content-factory | AI 内容工厂 | 内容与 AI | `/ai-content-factory` | P1 | 已迁移 |
...
| video-editor | 视频编辑 | 内容与 AI | `/video-editor` | P0 | 已迁移 |
...
- “菜单不全、页面不全”的问题，本轮已先从结构层解决：旧壳保留页面基线维持 `35` 个，但新壳当前菜单已按实际产品范围收口到 `34` 个页面。
- “功能不全”的问题还没有结束，下一步不再跳着做，而是严格按上表顺序逐页做 1:1 深度迁移与代码转换。
- 后续所有页面统一约束：不得只迁大意、不得只换标题、不得用占位交互替代旧壳真实结构与关系。
- 下一批优先页：`visual-lab`、`profit-analysis`、`traffic-board`，并按同一 1:1 深迁标准连续推进。
